...
def render_progress_charts():
    ...
    # Convert workout objects to DataFrame with proper datetime handling
    df = pd.DataFrame([
        {
            'date': w.date,
            'exercise': w.exercise,
    ...
    # Weekly summary
    if not df.empty:
        weekly_stats = df.resample('W')['duration'].sum().reset_index()
        ...
        with col1:
            ...
            recent_workouts = len(df[df.index > (datetime.now() - timedelta(days=7))])
            st.metric(
                "Total Workouts",
                total_workouts,
